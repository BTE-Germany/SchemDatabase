...
public class TrainsScreen {
    ...
    public TrainsScreen(SchemDatabase schemDatabase, Player pPlayer) {
    ...
    void createTrainsScreen() {

        try {
            this.schematicCount = MySQL.getSchematicsCountInCategory("trains");
        ...
        for (int i = 0; i < Math.ceil((double) this.schematicCount/ (double) this.maxPerPage); i++) {
            Inventory inv1 = Bukkit.createInventory(null, 54, "§cTrains§r §8- Page " + (i+1));
            this.getData(i);
    ...
    private void getData(int page) {
        ...
        try {
            ResultSet schemRS = MySQL.getPageSchematicsByCategory("trains", page, this.maxPerPage);
            while (true) {
                try {
