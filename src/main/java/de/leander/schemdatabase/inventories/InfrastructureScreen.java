...
public class InfrastructureScreen {
    ...
    public InfrastructureScreen(SchemDatabase schemDatabase, Player pPlayer) {
    ...
    void createInfrastructureScreen() {

        try {
            this.schematicCount = MySQL.getSchematicsCountInCategory("infrastructure");
        ...
        for (int i = 0; i < Math.ceil((double) this.schematicCount/ (double) this.maxPerPage); i++) {
            Inventory inv1 = Bukkit.createInventory(null, 54, "§cInfrastructure§r §8- Page " + (i+1));
            this.getData(i);
    ...
    public PagedInventory getPagedInv() {
        if (this.pagedInv == null) {
            this.createInfrastructureScreen();
        }
    ...
    private void getData(int page) {
        ...
        try {
            ResultSet schemRS = MySQL.getPageSchematicsByCategory("infrastructure", page, this.maxPerPage);
            while (true) {
                try {
