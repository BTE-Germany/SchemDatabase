...
public class MiscScreen {
    ...
    public MiscScreen(SchemDatabase schemDatabase, Player pPlayer) {
    ...
    void createMiscScreen() {

        try {
            this.schematicCount = MySQL.getSchematicsCountInCategory("misc");
        ...
        for (int i = 0; i < Math.ceil((double) this.schematicCount/ (double) this.maxPerPage); i++) {
            Inventory inv1 = Bukkit.createInventory(null, 54, "§cMiscellaneous§r §8- Page " + (i+1));
            this.getData(i);
    ...
    private void getData(int page) {
        ...
        try {
            ResultSet schemRS = MySQL.getPageSchematicsByCategory("misc", page, this.maxPerPage);
            while (true) {
                try {
