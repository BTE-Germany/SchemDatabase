...
public class TreesScreen {
    ...
    public TreesScreen(SchemDatabase schemDatabase, Player pPlayer) {
    ...
    void createTreesScreen() {

        try {
            this.schematicCount = MySQL.getSchematicsCountInCategory("trees");
        ...
        for (int i = 0; i < Math.ceil((double) this.schematicCount/ (double) this.maxPerPage); i++) {
            Inventory inv1 = Bukkit.createInventory(null, 54, "§cTrees§r §8- Page " + (i+1));
            this.getData(i);
    ...
    private void getData(int page) {
        ...
        try {
            ResultSet schemRS = MySQL.getPageSchematicsByCategory("trees", page, this.maxPerPage);
            while (true) {
                try {
